...
export function InfoCard(props) {
  ...
  return (
        <Card className={className}>
            ...
            <div className='font-medium text-center text-xl pb-4  dark:text-white'>{siteConfig('AUTHOR')}</div>
            <div className='text-sm text-center dark:text-white'>{siteConfig('BIO')}</div>
            <MenuGroupCard {...props} />
            <SocialButton />
        </Card>
  )
